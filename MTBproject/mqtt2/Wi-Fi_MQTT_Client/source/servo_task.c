...
#define SERVO_1_CORRECTION 5
#define SERVO_2_CORRECTION -1
#define SERVO_3_CORRECTION 0

QueueHandle_t servo_angles_q;


void servoRotate(cyhal_pwm_t *pwm_obj, int degree, int correction)
{
	float pulseWidthPerDegree = ((float)SERVO_TIME_RANGE/(float)SERVO_MAXIMUM_ROTATION_DEGREES);
	int pulseWidth = MIDLE_PULSE_WIDTH - (int)((degree + correction) * pulseWidthPerDegree);
...
void servo_task(void *pvParameters)
{
	...
	while (true)
	{
		...
		{
			servoRotate(&my_pwm_obj1, servo_angles_q_data.angle1, SERVO_1_CORRECTION);
			servoRotate(&my_pwm_obj2, servo_angles_q_data.angle2, SERVO_2_CORRECTION);
			servoRotate(&my_pwm_obj3, servo_angles_q_data.angle3, SERVO_3_CORRECTION);
		}
	}
}
