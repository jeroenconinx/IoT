...
void geelen_transform(float x_angle, float y_angle, int height, int *angle_1, int *angle_2, int *angle_3)
{
	...
	// rotation matrix
	  ...
	  *angle_1 = (int) (calcAngle(S1transformed[0], S1transformed[2])*RAD_TO_DEG);
	  *angle_2 = (int) (calcAngle(S2transformed[0], S2transformed[2])*RAD_TO_DEG);
	  *angle_3 = (int) (calcAngle(S3transformed[0], S3transformed[2])*RAD_TO_DEG);
}
...
void pid(int target_x, int target_y, int pos_x, int pos_y, int *angle_1, int *angle_2, int *angle_3)
{
	...
	float angle_y = kP * error_x + kI * integrator_error_x + kD * change_error_x;
	float angle_x = kP * error_y + kI * integrator_error_y + kD * change_error_y;

	geelen_transform(angle_x, angle_y, HEIGHT, angle_1, angle_2, angle_3);
	previous_pos_error_x = error_x;
	previous_pos_error_y = error_y;
}
